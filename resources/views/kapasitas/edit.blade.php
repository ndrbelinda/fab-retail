{{-- resources/views/kapasitas/edit.blade.php --}}
<x-layout>
    <x-slot:title>
        <div class="flex items-center">
            ...
            <!-- Judul -->
            <span>Ubah Kapasitas</span>
        </div>
    </x-slot:title>

    <!-- Form Input -->
    <form action="{{ route('kapasitas.update', $kapasitas->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="id_produk" id="id_produk" class="mt-1 block w-64 rounded-md border-gray-300 shadow-sm h-10 px-3 py-2 focus:border-blue-500 focus:ring-blue-500 @error('id_produk') border-red-500 @enderror" required>
                ...
                @foreach($produk as $item)
                    <option value="{{ $item->id }}" {{ $kapasitas->id_produk == $item->id ? 'selected' : '' }}>{{ $item->nama_produk }}</option>
                @endforeach
            </select>
            @error('id_produk')
        ...
        <div>
            ...
            <input type="text" name="besar_kapasitas" id="besar_kapasitas" value="{{ old('besar_kapasitas', $kapasitas->besar_kapasitas) }}" class="mt-1 block w-64 rounded-md border-gray-300 shadow-sm h-10 px-3 py-2 focus:border-blue-500 focus:ring-blue-500 @error('besar_kapasitas') border-red-500 @enderror" required
                max="999999999" 
                oninput="limitInputLength(this, 9)">
            @error('besar_kapasitas')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
            @enderror
            <div id="error-message-besar" class="text-sm text-red-600 mt-1 hidden">
        ...
        <div>
            ...
            <textarea name="deskripsi_kapasitas" id="deskripsi_kapasitas" rows="4" class="mt-1 block w-64 rounded-md border-gray-300 shadow-sm px-3 py-2 focus:border-blue-500 focus:ring-blue-500">{{ old('deskripsi_kapasitas', $kapasitas->deskripsi_kapasitas) }}</textarea>
        ...
        <div>
            ...
            <div class="mt-1">
                <label class="inline-flex items-center">
                    <input type="radio" name="tampil_ekatalog" value="1" class="form-radio text-blue-500 focus:ring-blue-500" {{ old('tampil_ekatalog', $kapasitas->tampil_ekatalog) == '1' ? 'checked' : '' }} required>
                    <span class="ml-2">Ya</span>
                </label>
                <label class="inline-flex items-center ml-6">
                    <input type="radio" name="tampil_ekatalog" value="0" class="form-radio text-blue-500 focus:ring-blue-500" {{ old('tampil_ekatalog', $kapasitas->tampil_ekatalog) == '0' ? 'checked' : '' }} required>
    ...
    <script>
        function limitInputLength(input, maxLength) {
            ...
            if (input.id === 'besar_kapasitas') {
                errorMessage = document.getElementById('error-message-besar');
            } else if (input.id === 'tarif_kapasitas') {
                errorMessage = document.getElementById('error-message-tarif');
            }
