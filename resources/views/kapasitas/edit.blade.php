...
<x-layout>
    ...
    <form action="{{ route('kapasitas.update', $kapasitas->id) }}" method="POST" class="space-y-4" onsubmit="return validateForm()">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="id_produk" id="id_produk" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border" required>
                ...
                @foreach($produk as $item)
                    <option value="{{ $item->id }}" {{ old('id_produk', $kapasitas->id_produk) == $item->id ? 'selected' : '' }}>{{ $item->nama_produk }}</option>
                @endforeach
            </select>
            @error('id_produk')
        ...
        <div>
            ...
            <input 
                ...
                id="besar_kapasitas" 
                value="{{ old('besar_kapasitas', $kapasitas->besar_kapasitas) }}" 
        ...
        <div>
            ...
            <textarea 
                name="deskripsi_kapasitas" 
                id="deskripsi_kapasitas" 
                rows="4" 
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2 border"
                maxlength="500"
                oninput="updateCharCounter(this)"
            >{{ old('deskripsi_kapasitas', $kapasitas->deskripsi_kapasitas) }}</textarea>
            <div class="text-xs text-gray-500 mt-1" id="char-counter">
                {{ strlen(old('deskripsi_kapasitas', $kapasitas->deskripsi_kapasitas)) }}/500 karakter
            </div>
        </div>
        ...
        <div>
            ...
            <div class="mt-1">
                <label class="inline-flex items-center">
                    <input type="radio" name="tampil_ekatalog" value="1" class="form-radio" {{ old('tampil_ekatalog', $kapasitas->tampil_ekatalog) == '1' ? 'checked' : '' }} required>
                    <span class="ml-2">Ya</span>
                </label>
                <label class="inline-flex items-center ml-6">
                    <input type="radio" name="tampil_ekatalog" value="0" class="form-radio" {{ old('tampil_ekatalog', $kapasitas->tampil_ekatalog) == '0' ? 'checked' : '' }} required>
    ...
    <div id="duplicateModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
            ...
            <div class="mt-4">
                <p class="text-sm text-gray-600" id="modalMessage">Kapasitas Internet sudah ada untuk produk ini.</p>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            updateCharCounter(document.getElementById('deskripsi_kapasitas'));
        });
