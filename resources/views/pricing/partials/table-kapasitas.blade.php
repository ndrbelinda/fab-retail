            ...
            @foreach($kapasitas as $item)
                <tr>
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900">
                        Rp{{ number_format($item->riwayatPricingKapasitas->sortByDesc('created_at')->first()->pricing ?? $item->tarif_kapasitas, 0, ',', '.') }}
                    </td>
    ...
    <div id="modal-riwayat-{{ $item->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-2xl mx-4 p-6" style="max-height: 80vh; overflow-y: auto;">
            <div class="flex justify-between items-start mb-4">
                <h3 class="text-lg font-semibold">Riwayat Pricing</h3>
            ...
            <div class="bg-gray-50 p-4 rounded-lg mb-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-500">Besar Kapasitas</p>
                        <p class="text-sm text-gray-900">{{ $item->besar_kapasitas }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">Tarif</p>
                        <p class="text-sm text-gray-900">Rp{{ number_format($item->tarif_kapasitas, 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($item->riwayatPricingKapasitas->sortByDesc('created_at') as $riwayat)
                            <tr>
...
@foreach($kapasitas as $item)
    <div id="modal-ubah-{{ $item->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md mx-4 p-6">
            <div class="flex justify-between items-start mb-4">
                <h3 class="text-lg font-semibold">Ubah Pricing Kapasitas</h3>
            ...
            <form action="{{ route('pricing.kapasitas.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="space-y-5">
                    {{-- Input Pricing --}}
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            ...
                            <input 
                                ...
                                value="{{ $item->riwayatPricingKapasitas->sortByDesc('created_at')->first()->pricing ?? $item->tarif_kapasitas }}"
