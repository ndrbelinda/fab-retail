...
<x-layout>
    ...
    <!-- Form Input -->
    <form action="{{ route('faq.update', $faq->id) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="id_produk" id="id_produk" class="mt-1 block w-64 rounded-md border-gray-300 shadow-sm h-10 px-3 py-2 focus:border-blue-500 focus:ring-blue-500 @error('id_produk') border-red-500 @enderror" required>
                ...
                @foreach($produk as $item)
                    <option value="{{ $item->id }}" {{ $faq->id_produk == $item->id ? 'selected' : '' }}>{{ $item->nama_produk }}</option>
                @endforeach
            </select>
            @error('id_produk')
        ...
        <div>
            ...
            <textarea name="pertanyaan" id="pertanyaan" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm px-3 py-2 focus:border-blue-500 focus:ring-blue-500" required>{{ old('pertanyaan', $faq->pertanyaan) }}</textarea>
            @error('pertanyaan')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea name="jawaban" id="jawaban" rows="5" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm px-3 py-2 focus:border-blue-500 focus:ring-blue-500" required>{{ old('jawaban', $faq->jawaban) }}</textarea>
            @error('jawaban')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <div class="mt-1">
                <label class="inline-flex items-center">
                    <input type="radio" name="tampil_ekatalog" value="1" class="form-radio text-blue-500 focus:ring-blue-500" {{ old('tampil_ekatalog', $faq->tampil_ekatalog) == '1' ? 'checked' : '' }} required>
                    <span class="ml-2">Ya</span>
                </label>
                <label class="inline-flex items-center ml-6">
                    <input type="radio" name="tampil_ekatalog" value="0" class="form-radio text-blue-500 focus:ring-blue-500" {{ old('tampil_ekatalog', $faq->tampil_ekatalog) == '0' ? 'checked' : '' }} required>
